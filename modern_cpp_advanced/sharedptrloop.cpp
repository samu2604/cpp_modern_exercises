...
class Object
{
  public:
    ...
    void threadLocalCall(size_t thread=0) {
      ++threadLocalCounter[thread];
    }
};
...
//------------------------------------------------------------------------------
void loopPassingSharedPtrByValue (int numThreads, size_t threadNo, long long& result)
{
...
//------------------------------------------------------------------------------
void loopPassingSharedPtrByRef (int numThreads, size_t threadNo, long long& result)
{
  Object::threadLocalCounter[threadNo] = 0;
  for (int i = 0; i < numIterations/numThreads; ++i) {
    if (i % printInterval == 0) {
...
//------------------------------------------------------------------------------
void loopPassingSharedPtrByRawPtr (int numThreads, size_t threadNo, long long& result)
{
  Object::threadLocalCounter[threadNo] = 0;
  for (int i = 0; i < numIterations/numThreads; ++i) {
    if (i % printInterval == 0) {
...
{
  if (argc < 3 || (argc == 3 && *argv[2] != 'v' && *argv[2] != 'r' && *argv[2] != 'p')) {
    std::cout << "Usage: "<< argv[0] << " N v|r|p [numIterations] [numElements]" << '\n';
  ...
  char style = argv[2][0];
  std::cout << "****************************************************************************\n";
  std::cout << "* Start " << numThreads << " thread(s) and pass std::shared_ptr ";
  switch (style) {
    case 'v':
      std::cout << "by value" << '\n';
      break;
  ...
  std::cout << "* and measure " << numIterations << " iterations over " << numElements << " elements\n";
  ...
  for (size_t threadIdx = 0; threadIdx < numThreadsU; ++threadIdx) {
    switch (style) {
      case 'v':
        threads.push_back(std::thread(loopPassingSharedPtrByValue,
                                      numThreads, threadIdx,
                                      std::ref(results[threadIdx])));
        break;
      case 'r':
        threads.push_back(std::thread(loopPassingSharedPtrByRef,
                                      numThreads, threadIdx,
                                      std::ref(results[threadIdx])));
        break;
      case 'p':
        threads.push_back(std::thread(loopPassingSharedPtrByRawPtr,
                                      numThreads, threadIdx,
                                      std::ref(results[threadIdx])));
        break;
    }
